...
int __bch2_xattr_set_inner(struct btree_trans *trans, subvol_inum inum,
			   ...
			   int type, int flags)
{
	...
	return bch2_hash_set(
		trans, bch2_xattr_hash_desc, hash_info, inum, &xattr->k_i,
		(flags & XATTR_CREATE ? STR_HASH_must_create : 0) |
			(flags & XATTR_REPLACE ? STR_HASH_must_replace :
						 0));
}
		   ...
		   int type, int flags)
{
	...
	if (value) {
		return __bch2_xattr_set_inner(trans, inum, hash_info, name,
					      value, size, type, flags);
	} else {
